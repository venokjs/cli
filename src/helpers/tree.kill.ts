...
export function treeKillSync(pid: number, signal?: string | number): void {
  ...
  getAllChilds(pid).forEach(function (pid) {
    killPid(pid, signal);
  });
...
function getAllPid(): { pid: number; ppid: number }[] {
  ...
  return rows
    ...
    .filter(<T>(input: null | undefined | T): input is T => input != null);
}

function getAllChilds(pid: number) {
  const ppidHash: { [key: number]: number[] } = {};

  const result: number[] = [];

  getAllPid().forEach(function (item) {
