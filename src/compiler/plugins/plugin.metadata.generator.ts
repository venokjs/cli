...
export class PluginMetadataGenerator {
  ...
  private traverseAndPrintMetadata(programRef: ts.Program, visitors: Array<ReadonlyVisitor>, outputDir: string, filename?: string) {
    ...
    const collectedMetadata: Record<string, Record<string, Array<[ts.CallExpression, DeepPluginMeta]>>> = {};

    visitors.forEach((visitor) => {
      collectedMetadata[visitor.key] = visitor.collect() as Record<string, Array<[ts.CallExpression, DeepPluginMeta]>>;
      typeImports = {
        ...typeImports,
        ...visitor.typeImports,
      };
