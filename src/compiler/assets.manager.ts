...
export class AssetsManager {
  ...
  public copyAssets(configuration: Required<Configuration>, appName: string, outDir: string, watchAssetsMode: boolean) {
    const assets = getValueOrDefault<Asset[]>(configuration, "compilerOptions.assets", appName) || [];
    ...
    try {
      ...
      const isWatchEnabled = getValueOrDefault<boolean>(configuration, "compilerOptions.watchAssets", appName) || watchAssetsMode;
      ...
      throw new Error(`An error occurred during the assets copying process. ${(err as any).message}`);
    }
  }
